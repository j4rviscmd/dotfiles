-- 'autoread'をOSファイルウォッチャー(FSEvents/inotify)でリアルタイム検知させるモジュール
-- Why: tmux非フォーカスpaneではFocusGained/CursorHoldが発火せず、外部変更(Claude Code等のAIエージェント編集)が
--      フォーカスするまでバッファに反映されないため、event駆動の常時監視が必要
-- 出所: Neovim本家 runtime/lua/nvim/autoread.lua (PR #37971, Apache 2.0) をv0.12.5互換に移植
--      差分は vim._core.util.nvim_on (0.13 API) を nvim_create_autocmd に置換、
--      およびテスト用ヘルパー(M._set_debounce / M._is_watching)の削除のみ
-- TODO: Neovim 0.13リリース後はネイティブで同等機能が働くため、本ファイルとinit.luaの呼び出しを削除する
...
-- Why: 100msは本家PR #37971指定の値(連続fsイベント例: truncate+writeを1回のchecktimeへ集約)。変更時注意: 本家実装と値を変えると移植元の検証済み挙動から乖離する
